...
from Object_Detector_Module import object_detection
from FaceRecognitionModule import Face_recognition
    ...
    def run(self):
        ...
        while not should_stop:
            ...
            if k == ord('l'):
                ...
                # Hand_Gestures Controls
            hands, frameRet = detector.findHands(frameRet)  # With Draw
            if hands:
                # Hand 1
                hand1 = hands[0]
            ...
            if OVERRIDE:
                object_detection(frameRet)
                Face_recognition(frameRet)
            ...
            if self.send_rc_control and not OVERRIDE:
                for (x, y, w, h) in faces:
                    ...
                    cv2.imwrite(f'Resources/detect_Personel/{time.time()}.jpg', frameRet)
                    # time.sleep(0.3)
                ...
                if noFaces:
                    ...
                    self.up_down_velocity = 0
                    self.for_back_velocity = 0
                    # warning = print("NO TARGET")
                    cv2.putText(frameRet, 'No TARGET', (40, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            ...
            if OVERRIDE:
                show = "Survelliance: Drone_Speed: {}".format(oSpeed)
                dCol = (255, 255, 255)
            else:
                show = "Face_Tracking:Current_Distance {}".format(str(tDistance))
